...
c	It is a standard Matlab SYSTEM FUNCTION in the format required by
...
c	NOTE:	 	For version 4.x of Matlab.  WILL NOT work with earlier 
c			versions.  See te_4.f for code compatible with version 3.5.
...
C IN ALL FORTRAN MEX FILES for version 4.x of MATLAB.
C---------------------------------------------------------------------
C
...
c		constants for the SIZES vector.
	...
	double precision 	rx(nx),rx0(nx),dxdt(nx),sizes(6)
	double precision	rt,ry(ny),ru(nu)
...
c		Check the number of input arguments
	...
	if (nrhs .ne. 5) then
		call mexErrMsgTxt('TE_4 needs 5 input variables.')
	else
...
c			inputs to local storage.
     ...
     +			('U must be a vector, length '//lenu)
			...
			u=mxGetPr(prhs(3))
			call mxCopyPtrToReal8(u,ru,nu)		! copy u into local storage
		else if (iflag.eq.1 .or. iflag.eq.3) then
			call mexErrMsgTxt('U must be supplied.')
		end if
		
		m=mxGetM(prhs(5))		! Get size of "p1" array.
		n=mxGetN(prhs(5))
		if (m.eq.1 .and. n.eq.np1 .or. m.eq.np1 .and. n.eq.1) then
			p1=mxGetPr(prhs(5))
			call mxCopyPtrToReal8(p1,dp1,np1)		! copy p1 to local REAL*8 array 
 		else
			call mexErrMsgTxt
     +		('P1 must be a vector, length 50')
		end if
...
c		If flag=3, calculate outputs
			...
			call mxCopyReal8ToPtr(ry,mxGetPr(plhs(1)),ny)  	! returns y to Matlab
		end if		
	else
...
c		Save certain variables for next call.
	...
	call mxCopyReal8ToPtr(dp1,p1,np1)
